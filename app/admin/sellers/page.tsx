    ...
    <main className="min-h-screen bg-stone-100 p-4 sm:p-6">
      <div className="mx-auto max-w-6xl">
        <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
          ...
          <Link href="/admin/moderation" className="rounded-md border border-stone-300 px-3 py-2 text-sm font-semibold">
            Listing moderation
          </Link>
        </div>
        {error && <p className="mb-4 rounded-md bg-red-50 p-3 text-red-800">{error}</p>}
            ...
            <article key={item.id} className="rounded-lg border border-stone-200 bg-white p-4 shadow-sm">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  <p className="text-sm text-stone-500">{item.user.email} · {item.status}</p>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {item.documents.map((document: any) => (
                  <a
                    key={document.id}
                    href={document.url}
                    className="rounded-md bg-stone-100 px-3 py-2 text-xs font-semibold"
                  >
